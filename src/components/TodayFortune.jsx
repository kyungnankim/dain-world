// src/components/TodayFortune.jsx
...
const TodayFortune = ({ totalPhotos }) => {
  const [fortune, setFortune] = useState(
    "버튼을 눌러 오늘의 운세를 확인하세요!"
  );
  const [imageNumber, setImageNumber] = useState(1);

  // ✨ 디버깅용: 컴포넌트가 받는 totalPhotos 값을 확인합니다.
  useEffect(() => {
    console.log("TodayFortune 컴포넌트가 받은 사진 총 개수:", totalPhotos);
  }, [totalPhotos]);
  ...
  const showFortune = () => {
    ...
    if (totalPhotos > 0) {
      const randomImageNumber = Math.floor(Math.random() * totalPhotos) + 1;
      setImageNumber(randomImageNumber);

      // ✨ 디버깅용: 새로 생성된 이미지 경로를 확인합니다.
      const newImageUrl = `/images/dain_${randomImageNumber}.jpg`;
      console.log("새로 선택된 이미지 경로:", newImageUrl);
    }
  };

  const fortuneImageUrl = `/images/dain_${imageNumber}.jpg`;

  return (
    <div className="card fortune-card">
      <h2>✨ 다인이의 오늘의 운세 ✨</h2>
      <img
        // ✨ 중요: key 속성을 추가하여 사진이 바뀔 때마다 이미지를 새로 그리도록 합니다.
        key={fortuneImageUrl}
        src={fortuneImageUrl}
        alt="다인이 운세 사진"
